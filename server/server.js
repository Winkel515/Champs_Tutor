...
const sharedProperties =
  'name _id rating price subjects description profileImage '; // Edit shared properties here
...
app.get('/tutors', (req, res) => {
  Tutor.find({ showTutor: true }, `${sharedProperties}` + 'reviews')
    .then((tutors) => {
      res.json({ tutors });
    })
    .catch((e) => {
...
app.post(
  ...
  (req, res) => {
    var body = _.pick(req.body, [
      ...
      'facebook',
      'profileImage',
    ]); // On sign-up, tutors will input email, name and password.
    body.subjects = body.subjects.split(',');
    if (req.file) {
...
app.patch('/tutors/me', authenticate, (req, res) => {
  const editList = [
    ...
    'profileImage',
    'filePath',
  ]; // Array to store properies that can be edited by the tutor
...
app.post('/tutors/login', (req, res) => {
  ...
  Tutor.findByCredentials(body.email, body.password)
    .then((tutor) => {
...
app.post('/tutors/:tutorId/reviews', (req, res) => {
  // Work in progress
